   ...
   <form action="" method="post">
      <div class="col-md-12" id="middle_reservation_cont_in">
            <div class="form-group">
               ...
               <?php 
                  ...
                  while($row2 = mysqli_fetch_array($run2)){
                     $id            = $row2['id'];
                     $city_name_rus = $row2['city_name_rus'];
                     echo '<option value="'.$id.'">' .$city_name_rus.'</option>';
                  }
               ?>
               </select>
            </div>
            ...
            <select class="form-control" name="tour_genre">
               <?php 
                  ...
                  while($row = mysqli_fetch_array($run)){
                     $id             = $row['id'];
                     $genre_name_rus = $row['genre_name_rus'];
                     echo '<option value="'.$id.'">' .$genre_name_rus.'</option>';
                  }
               ?>
            </select>
         </div>
         ...
         <div class="col-md-6" id="res_bottom_right">
            <div class="form-group">
               <label>Фамилия</label>
               <input type="text" name="lname" class="form-control" placeholder="Jolly" autocomplete="off">
            </div>
         </div>
         <div class="col-md-6" id="res_bottom_left">
            <div class="form-group">
         ...
         <div class="form-group">
            <a href="res"><input type="submit" name="submit" value="Бронирование" class="btn btn-success" id="main_submit">
         </div>
      </div>
   </form>
<?php
      ...
      if(isset($_POST['submit'])){
         ...
         if(!empty($tel) or !empty($email) or !empty($fname) or !empty($lname)){
          ...
          $message = '
            ...
            header('location: booking_personal_ru.php?city='.$city.'&tour_genre='.$tour_genre.'&dateFrom='.$dateFrom.'&dateTo='.$dateTo.'&adultNumber='.$adultNumber.'&childNumber='.$childNumber.'&datee='.$datee);
